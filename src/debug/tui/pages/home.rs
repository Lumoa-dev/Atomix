...
impl HomePage {
    ...
    fn display_lines(&self, session: &LocalDebugSession) -> Vec<HomeLine> {
        ...
        for phase in ExecutionPhase::all() {
            let steps: Vec<&crate::debug::trace::StepRecord> =
                trace.steps.iter().filter(|s| s.phase == *phase).collect();
            let step_count = steps.len();
            let is_expanded = self.expanded_phases.contains(phase);
            lines.push(HomeLine::PhaseHeader {
                phase: *phase,
                count: step_count,
                ...
                match line {
                    HomeLine::PhaseHeader {
                        ...
                        Line::from(Span::styled(
                            format!(" {}  {} 段（{} Step）", symbol, phase.name(), count),
                            style,
                        ))
                    }
                    HomeLine::Step(step) => {
                        ...
                        let elapsed = if step.elapsed_us >= 1000 {
                            format!("{:.1}ms", step.elapsed_us as f64 / 1000.0)
                        } else {
    ...
    fn on_enter(&mut self, session: &mut LocalDebugSession, status: &mut String) {
        ...
        match &lines[self.selected] {
            HomeLine::PhaseHeader {
                ...
                if *expanded {
                    self.expanded_phases.retain(|p| p != phase);
                } else {
                    self.expanded_phases.push(*phase);
                }
            }
            HomeLine::Step(step) => {
