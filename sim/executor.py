...
class ExecutorSync:
    ...
    def __init__(self, hardware: HardwareModel, slot_manager: SlotManager):
        ...
        # v0.3：负载均衡器引用（由 Simulation 注入）
        self.load_balancer = None

        # 运行中的任务 {task_id: (Task, Slot, remaining_cpu_ms, executor_id)}
    ...
    def dispatch(self, task: Task, slot: Slot, executor_id: int = 0):
        """分配任务到槽位（v0.3：接受 executor_id 用于负载均衡追踪）"""
        task.slot_id = slot.slot_id
        task.mem_addr = slot.start_addr
        task.mem_size = slot.size_mb
        task.status = "RUNNING"
        ...
        self.running[task.task_id] = (task, slot, task.cpu_ms, executor_id)
    ...
    def step(self, dt_ms: float, sim_time: float) -> List[Task]:
        ...
        for task_id in list(self.running.keys()):
            task, slot, remaining_ms, executor_id = self.running[task_id]
            ...
            if remaining_ms <= 0:
                ...
                if self.load_balancer is not None:
                    self.load_balancer.task_done(executor_id)
                ...
                self.running[task_id] = (task, slot, remaining_ms, executor_id)
    ...
    def running_count(self) -> int:
        return sum(1 for _, (t, _, _, _) in self.running.items() if t.status == "RUNNING")

    @property
    def suspended_count(self) -> int:
        return sum(1 for _, (t, _, _, _) in self.running.items() if t.status == "SUSPENDED")

    def resume_suspended(self, sim_time: float):
        """尝试恢复挂起的任务"""
        for task_id, (task, slot, remaining_ms, executor_id) in list(self.running.items()):
            if task.status == "SUSPENDED":
                # 检查是否有空闲槽位可以容纳
                task.status = "RUNNING"
