...
from sim.visualizer import (
    ...
    plot_dashboard, plot_new_factors, plot_load_distribution
)
...
class ReportGenerator:
    ...
    def __init__(self, output_dir: str = "sim/reports", detailed: bool = False):
        """
        detailed: 是否生成细粒度图（时间序列、因子分解等）。
                 默认只生成汇总对比图（吞吐量、热力图、帕累托前沿）。
        """
    ...
    def generate_scenario_report(self, scenario_name: str,
        ...
        if self.detailed:
            ...
            if best_label and best_label in results:
                ...
                plot_new_factors(
                    results[best_label].get_arrays(),
                    os.path.join(scenario_dir, "new_factors.png")
                )
                plot_load_distribution(
                    results[best_label].get_arrays(),
                    os.path.join(scenario_dir, "load_distribution.png")
                )
