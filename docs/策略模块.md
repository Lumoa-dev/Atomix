...
| 参数 | 默认值 | 含义 |
|------|--------|------|
| `loop_multiplier` | 10 | 循环内分配乘数 |
| `recursion_depth_limit` | 8 | 递归调用最大深度 |
| `unknown_alloc_fallback` | 1 MB | 无法静态推导的分配量 |
| `stack_overhead_factor` | 1.1 | 栈帧安全系数 |

所有参数可通过 `.task` 段编译指令覆盖。开发者如果明确知道循环会跑 5000 次，应该主动标注 `@loop_iterations(5000)` 以获得更准确的槽位分配。
