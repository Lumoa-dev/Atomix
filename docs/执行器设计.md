...
```
1. 完成当前指令              ← 不允许半条指令状态
2. 保存完整寄存器上下文       ← 16×64 位 → 128 字节
3. 保存 PC                   ← 当前指令偏移
4. 设置 task->state = SUSPENDED
5. 通知内存管理器：task_id 需要扩容
6. 执行器切换到下一就绪任务
```
...
```
1. 内存管理器将 task 标记为 RESUMED
2. 批次管理器将 task 放回就绪队列
3. 执行器下次取到 task 时：
   a. 恢复寄存器上下文
   b. 恢复 PC
...
执行中
  ...
  ├── ECALL alloc(size)
  │     │
  │     ├── check_watermark → OK → 分配成功，继续执行
  │     │
  │     └── check_watermark → 超警戒线
  │           │
  │           ├── 1. 保存上下文 (regs + PC)
  │           ├── 2. task->state = SUSPENDED
  │           ├── 3. 通知内存管理器扩容
  │           ├── 4. 通知风险管控（OOM 计数 +1）
  │           └── 5. yield → 下一任务
  │
  ...
  │     ├── 更新 task->mem_addr
  │     ├── 更新 task->watermark
  │     └── task->state = RESUMED → 放回就绪队列
  │
  └── 执行器再次取到任务
        ├── 恢复上下文
        ├── task->state = RUNNING
        └── 从 ECALL alloc 重新执行（此时水位已更新，分配成功）
```
