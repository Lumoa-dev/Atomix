...
| **IR (中间表示)** | 32 位定长指令序列，52 条 opcode，16 个虚拟寄存器 | 02-指令集规范.md |
| **并发额度 N_batch** | 动态计算的最优并发数 = min(H, S) | 策略模块.md §10 |
...
整个实现分为 4 个里程碑，每个里程碑有明确的前置依赖和交付物。

```mermaid
flowchart LR
    P1["Phase 1<br/>语言核心与编译器"] --> P2["Phase 2<br/>虚拟机与指令执行"]
    P2 --> P3["Phase 3<br/>运行时与调度系统"]
    P3 --> P4["Phase 4<br/>工具链与标准库"]
```
...
| **关键设计文档** | 编译管线.md、编译行为.md、通用语法.md、类型系统.md、区外语法.md、02-指令集规范.md |
| **预估需求数** | ~120 条 |
| **预估工作量** | 较大（这是基础，后续全部依赖此阶段） |

### 2.2 Phase 2 — 虚拟机与指令执行

| 项目 | 内容 |
|------|------|
| **目标** | 实现完整的 VM 执行引擎：IR 加载 → 指令解码 → 52 条指令执行 → 内存管理 → 沙箱隔离 → ECALL 接口 |
| **前置依赖** | Phase 1（需要 .atxe 格式加载和 IR 指令定义） |
| **交付物** | `atomix runner run` 子命令（dev 模式单次执行）、VM 库 `atomix-vm`、ECALL 宿主接口 |
| **关键设计文档** | 02-指令集规范.md、执行器设计.md、运行时架构.md（第7节） |
| **预估需求数** | ~100 条 |
| **预估工作量** | 大（指令实现量大但每条指令逻辑简单） |
...
| **交付物** | `atomix runner` 二进制、运行时库 `atomix-runtime`、自适应控制器 `atomix-controller`、.atxe 任务的调度执行 |
| **关键设计文档** | 运行时架构.md（全文）、策略模块.md（全文）、配置设计.md（全文）、执行器设计.md§3-6 |
| **预估需求数** | ~120 条 |
| **预估工作量** | 最大（涉及并发、内存管理、自适应控制等复杂算法） |
...
| **交付物** | 完整 `atomix` CLI（含 test/debug/public/install/format 等）、`atomix runner`（含热注入监听）、标准库 `.atxo` 文件、包索引系统 |
| **关键设计文档** | 外围工具.md、包管理.md、标准库.md、内置函数.md、语法设计/附录/* |
| **预估需求数** | ~90 条 |
| **预估工作量** | 中（主要是外围系统和集成工作） |
...
| 使用范围 | 仅 Runner 模式、ECALL 阻塞操作、网络 I/O。**VM 执行循环本身必须是同步的**（见 执行器设计.md §2） |
...
| **优先级** | P2 |
| **依赖** | P1-LNK-005 |
| **描述** | 编译器在生成 IR 后、输出 .atxe 前执行内存静态分析。.atxe 文件头新增 `memory_profile` 字段：code_mb（精确，.text 段大小）、rodata_mb（精确，.rodata 段大小）、stack_mb（上界，调用图 DFS）、heap_mb（上界，CFG 路径遍历）、peak_mb（code+rodata+max(stack,heap)）。算法详见策略模块.md §9.6。 |
...
| **验收标准** | dispatch_table 有 256 个条目。已分配 52 条 opcode 每个对应正确的处理函数。未分配的 204 个 opcode 指向 illegal_instruction 处理。4 种编码模板的 operand 提取正确。 |
...
| **描述** | 确保全部 **52 条指令**均已实现，与 02-指令集规范.md §6 的完整编码速查表一一对应。无遗漏。 |
| **验收标准** | 对速查表中每条 opcode 运行单元测试验证其行为符合操作摘要。52 条 opcode 的 dispatch_table 处理函数均有实现。 |
...
| 字段 | 内容 |
|------|------|
| **原文档引用** | 运行时架构.md §5.4.1；策略模块.md §8 |
| **优先级** | P1 |
| **依赖** | P2-MEM-004 |
| **描述** | 槽位布局中预留空白滑道（默认 1.5× 以上）。OOM 发生时任务滑入滑道→空出死区→后续邻居合并后重新切分槽位。滑道大小动态调节（推荐动态弹性方案详见策略模块.md §8.1）。 |
...
| 字段 | 内容 |
|------|------|
| **原文档引用** | 执行器设计.md §8；02-指令集规范.md §3.8 |
| **优先级** | P0 |
| **依赖** | P2-MEM-001 |
...
| **优先级** | P0 |
| **依赖** | P2-VM-001 |
| **描述** | 任务进入系统后先进入任务池（磁盘）。每个任务条目记录：task_id（唯一标识）、disk_addr（磁盘存储地址）、mem_addr（运行时填充，仅在 RUNNING 期间有效）、status（状态码）、counter（执行计数，可选）。双地址管理：disk_addr 从任务入池到执行完毕卸载；mem_addr 仅在 RUNNING 期间有效。 |
| **验收标准** | 新任务入池写入磁盘。disk_addr 在任务执行完毕后标记为可清理。mem_addr 仅在 RUNNING 期间有效。任务池可持久化到磁盘。 |
...
| 字段 | 内容 |
|------|------|
| **原文档引用** | 运行时架构.md §4.2；配置设计.md §4 |
| **优先级** | P0 |
| **依赖** | P3-BM-001 |
| **描述** | `C = (CPU_cores × α_cpu) / CPU_per_task`；`M = (MEM_free × α_mem) / MEM_per_task`；`I = (IOPS_avail × α_io) / IOPS_per_task`；`N = (NET_avail × α_net) / NET_per_task`。H = `⌊min(C, M, I, N)⌋`。硬件参数通过 OS 查询获取。α_* 保留系数和每任务资源估算可通过 Runner 配置覆盖。 |
| **验收标准** | H 为 4 个维度计算结果的最小值向下取整。任一维度为 0 时 H=0（无法执行）。OS 查询返回合法硬件参数。配置覆盖优先于检测值。 |

##### P3-BM-003 软上限 S 与因子调节

| 字段 | 内容 |
|------|------|
| **原文档引用** | 运行时架构.md §4.3；策略模块.md §5、§6 |
| **优先级** | P0 |
| **依赖** | P3-BM-001 |
| **描述** | S 在 H 的基础上受四个动态因子调节：积压 β、速度 λ、体积 σ、方差 γ。因子使用连续 S 型函数（非离散分段）。因子合并采用加权几何平均（非乘法链）。详见策略模块.md §5-§6。 |
...
| **优先级** | P1 |
| **依赖** | P3-BM-003 |
| **描述** | `v_t = σ_t / μ_t`，`γ(v_t) = 0.50 + 0.55 / (1 + e^{5.0×(v_t−0.5)})`。v_t→0 时 γ→1.05，v_t=0.5 时 γ=0.78，v_t→∞ 时 γ→0.50。 |
...
| **优先级** | P0 |
| **依赖** | P3-PL-001 |
| **描述** | 主循环：取下一就绪任务 → 恢复上下文 → 执行一个时间片 → 保存上下文 → 任务 DONE？释放槽位/更新状态 → OOM_PAUSE？管理器扩容 → 其他？放回就绪队列。 |
...
| **优先级** | P0 |
| **依赖** | P3-SCH-001 |
| **描述** | 上下文切换时机：QUANTUM 耗尽（协作式让出）、ECALL 阻塞（阻塞式挂起）、OOM（紧急挂起）、TASK_JOIN（阻塞式挂起）、TASK_RET（终止）。切换开销：保存/恢复各 ~18 条 MOV 指令（16×64 位寄存器 + PC + SP/FP = 144 字节）。零拷贝上下文——寄存器和 PC 保存在任务的 TaskContext 结构体中，切换不涉及 memcpy。 |
| **验收标准** | 上下文保存后恢复能回到相同的执行状态。向量/结构体保存完整。切换开销在 36 条 MOV 指令内。 |
...
| 字段 | 内容 |
|------|------|
| **原文档引用** | 运行时架构.md §8；02-指令集规范.md §3.6 |
| **优先级** | P0 |
| **依赖** | P3-PL-001 |
| **描述** | 调度器根据 .task 段预计算的依赖图层级，按从深到浅分批执行。最深（无依赖）→ 首批 → 逐层向上 → 根任务 → 最后。每层按可用额度（N_batch）分批执行。每层全部完成后结果向上传递。 |
| **验收标准** | 无依赖的子任务在父任务之前执行。同一层级任务可并发执行（额度允许范围内）。父任务在所有子任务完成后才继续。依赖图与 .task 段一致。 |

##### P3-SCH-005 依赖图调度与批次管理协同
...
| **优先级** | P1 |
| **依赖** | P3-RUN-001 |
| **描述** | 热注入协议：Compiler → Runner（TCP）：Header(8B) + IR Binary(variable)。Header：Magic "ATXR"(4B) + Version(2B) + Mode(1B) + Flags(1B)。Runner → Compiler：Status(2B) + TaskID(4B)（仅成功时）。Status：0=成功入池、1=格式错误、2=池满。注入的任务进入任务池（写入磁盘），参与调度。 |
...
| **验收标准** | `atomix --help` 显示所有子命令概览（详见 命令行规范.md）。`atomix build`、`atomix runner run`、`atomix test` 等子命令均可识别。 |
...
| **优先级** | P1 |
| **依赖** | P3-RUN-002 |
| **描述** | 实现 `atomix origin` 命令用于配置远程连接、`atomix task --origin` 用于控制远程任务。`atomix runner` 本身就有本地远程统一的执行入口——`atomix runner run <name>` 在本地执行，`atomix runner run <name> --origin prod` 在远程执行。参见 命令行规范.md。 |
...
| 编译行为.md | 4 | §1-§5 | Phase 1 |
| 编译管线.md | 35 | §1-§8 | Phase 1 |
| 运行时架构.md | 20 | §1-§11 | Phase 3 |
| 执行器设计.md | 12 | §1-§9 | Phase 2 |
| 配置设计.md | 3 | §1-§6 | Phase 3 |
| 策略模块.md | 18 | §1-§12 | Phase 3 |
| 外围工具.md | 12 | §1-§7 | Phase 4 |
| 通用语法.md | 15 | §1-§22 | Phase 1 |
| 类型系统.md | 8 | §1-§8 | Phase 1 |
| 区外语法.md | 8 | §1-§7 | Phase 1 |
| INPUT语法.md | 6 | §1-§8 | Phase 1 |
| OUT语法.md | 4 | §1-§6 | Phase 1 |
| TASK语法.md | 8 | §1-§8 | Phase 1 |
| WORKS语法.md | 6 | §1-§8 | Phase 1 |
| TOOLS语法.md | 2 | §1-§5 | Phase 1 |
| 内置函数.md | 1 | §1-§6 | Phase 4 |
| 包管理.md | 10 | §1-§9 | Phase 4 |
| 标准库.md | 5 | §1-§7 | Phase 4 |
| 关键字参考.md | 1 | 全文 | Phase 1 |
| 钩子与IS值参考.md | 3 | §1-§2 | Phase 2/3 |
| 默认装饰器参考.md | 2 | §B.1-B.6 | Phase 1/4 |
| 数据源地址与参数速查.md | 2 | §A.1-A.14 | Phase 1 |
| **合计** | **~400 条** | — | — |
...
| Phase | 分类 | 编号范围 | 数量(参考) |
|-------|------|---------|-----------|
| P1 | LEX 词法分析 | P1-LEX-001 ~ P1-LEX-006 | ~6 |
| P1 | SYN 语法分析 | P1-SYN-001 ~ P1-SYN-013 | ~13 |
| P1 | SEM 语义分析 | P1-SEM-001 ~ P1-SEM-012 | ~12 |
| P1 | IR 生成 | P1-IR-001 ~ P1-IR-008 | ~8 |
| P1 | OPT 优化 | P1-OPT-001 ~ P1-OPT-006 | ~6 |
| P1 | LNK 链接 | P1-LNK-001 ~ P1-LNK-006 | ~6 |
| P1 | ZON 编译行为 | P1-ZON-001 ~ P1-ZON-004 | ~4 |
| P2 | VM 核心 | P2-VM-001 ~ P2-VM-004 | ~4 |
| P2 | ISA 指令 | P2-ISA-001 ~ P2-ISA-010 | ~10 |
| P2 | MEM 内存 | P2-MEM-001 ~ P2-MEM-005 | ~5 |
| P2 | SBX 沙箱 | P2-SBX-001 | ~1 |
| P2 | EXN 异常 | P2-EXN-001 ~ P2-EXN-002 | ~2 |
| P3 | PL 任务池 | P3-PL-001 ~ P3-PL-002 | ~2 |
| P3 | BM 批次管理 | P3-BM-001 ~ P3-BM-012 | ~12 |
| P3 | SCH 调度 | P3-SCH-001 ~ P3-SCH-005 | ~5 |
| P3 | CS 冷启动 | P3-CS-001 | ~1 |
| P3 | RUN Runner | P3-RUN-001 ~ P3-RUN-002 | ~2 |
| P4 | CLI 命令行 | P4-CLI-001 ~ P4-CLI-007 | ~7 |
| P4 | PM 包管理 | P4-PM-001 ~ P4-PM-005 | ~5 |
| P4 | STDLIB 标准库 | P4-STDLIB-001 ~ P4-STDLIB-006 | ~6 |
| P4 | TST 测试 | P4-TST-001 ~ P4-TST-002 | ~2 |
| P4 | DBG 调试 | P4-DBG-001 | ~1 |
| **合计** | **22 个分类** | | **~120 条** |

> 当前版本已编写约 **120 条** 需求条目，覆盖 4 个 Phase 的核心功能。完整版应扩充至 350-500 条，按本规范的结构和粒度补充即可。
